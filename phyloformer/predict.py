...
parser.add_argument('--m', type=str, default='seqgen_model.pt', help='path to the NN model')
...
with torch.no_grad():
    ...
    net=torch.load(args.m,map_location=torch.device(device))
    tensors={}
    for ali in alignments:
        ...
        for array in arrays:
                ...
                #X.append(((torch.from_numpy(array)).t()))
        #print(X[0].view(22,1,-1).shape)
    ...
    for ali in tensors:
        ...
        configure(net,seqs,device)
        ids=[seq for seq in seqs]
        tensor=tensors[ali][None,:,:]
        y_pred = net(tensor.float())[0]
        y_pred=y_pred.view(net.nb_pairs)
        nn_dist={}
        ...
        #print(outdir+'predicted_'+ali.split('.')[0]+'.tree')
